export function renderDialogueView({ element, conversation, lines, lineIndex, showChoices, onAdvance, onChoose, onClose }) {
  ...
  element.querySelector("[data-dialogue-close]")?.addEventListener("click", onClose);
  element.querySelector(".dialogue-line")?.addEventListener("click", onAdvance);
  element.querySelectorAll("[data-dialogue-option]").forEach((button) => button.addEventListener("click", () => onChoose(button.dataset.dialogueOption)));
}

export function closeDialogueView(element) { element.hidden = true; element.innerHTML = ""; }
