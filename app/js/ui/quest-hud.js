export function buildQuestHudModel({ quest, placement = null, actionSlotId = null, locations = [], deadlines = [], now = Date.now() } = {}) {
  ...
  const distanceFromOrigin = placement ? Math.max(0, Math.round(placement.distanceFromOriginMeters ?? 0)) : null;
  const minimumDistanceFromOrigin = placement ? Math.max(0, Math.round(placement.minimumDistanceFromOriginMeters ?? 0)) : null;
  const ready = Boolean(actionSlotId && placement?.status === "ready");
  const clocks = deadlines.filter((deadline) => Number.isFinite(deadline?.expiresAt) && !deadline.completedAt && !deadline.failedAt && !deadline.endedAt).map((deadline) => ({ id: deadline.id, label: deadline.label ?? "Temps restant", remainingMs: Math.max(0, deadline.expiresAt - now), expired: deadline.expiresAt <= now }));
  return { id: quest.id, title: quest.title, summary: quest.description, objective: activeObjective?.text ?? (quest.choices?.length ? "Choisissez votre réponse" : "Quête terminée"), objectives: quest.objectives.map(({ id, text, state }) => ({ id, text, state })), choices: (quest.choices ?? []).map(({ id, label }) => ({ id, label })), locations: locations.map(({ id, name }) => ({ id, name })), deadlines: clocks, current, target, distanceFromOrigin, minimumDistanceFromOrigin, progressPercent: placement ? Math.min(100, current / target * 100) : null, ready, actionLabel: ready ? actionLabel(activeObjective, actionSlotId) : null };
}
...
export function renderQuestHud({ element, model, expanded = false, onToggle, onAction, onShowLocation, onChoice }) {
  if (!element) return;
  element.hidden = !model;
  if (!model) { element.replaceChildren(); return; }
  const originRequirement = model.minimumDistanceFromOrigin > 0 ? ` · éloignement ${model.distanceFromOrigin} / ${model.minimumDistanceFromOrigin} m` : "";
  const progress = model.progressPercent === null ? "" : `<div class="quest-hud__progress" role="progressbar" aria-label="Distance parcourue : ${model.current} sur ${model.target} mètres${originRequirement}" aria-valuemin="0" aria-valuemax="${model.target}" aria-valuenow="${model.current}"><span style="width:${model.progressPercent}%"></span><small>${model.current} / ${model.target} m parcourus${originRequirement}</small></div>`;
  ...
  const choices = model.choices?.map((choice) => `<button type="button" class="quest-choice-button" data-quest-choice="${escapeHtml(choice.id)}">${escapeHtml(choice.label)}</button>`).join("") ?? "";
  const deadlines = model.deadlines?.map((deadline) => `<p class="quest-hud__deadline${deadline.expired ? " is-expired" : ""}"><span aria-hidden="true">◷</span> ${escapeHtml(deadline.label)} : ${formatRemainingTime(deadline.remainingMs)}</p>`).join("") ?? "";
  ...
  element.querySelectorAll("[data-quest-choice]").forEach((button) => button.addEventListener("click", () => onChoice?.(button.dataset.questChoice)));
}
...
function formatRemainingTime(milliseconds) {
  const seconds = Math.max(0, Math.ceil(milliseconds / 1000)); const minutes = Math.floor(seconds / 60); const remainder = seconds % 60;
  return minutes > 0 ? `${minutes} min ${String(remainder).padStart(2, "0")} s` : `${remainder} s`;
}
