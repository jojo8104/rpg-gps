...
export function renderBattleResultView({
  element,
  battle,
  result,
  playerId,
    ...
    .map((entry) => {
      const promotion =
        entry.previousRank !== entry.rank
          ? ` · ${label(HERO_COMMAND_RANKS, entry.previousRank)} → ${label(HERO_COMMAND_RANKS, entry.rank)}`
          : "";
  ...
  const ownedUnitIds = new Set(
    battle.teams
      .find((team) => team.id === playerTeamId)
      ?.units.filter((unit) => unit.playerId === playerId)
      .map((unit) => unit.sourceId) ?? [],
  );
  const battleUnits = new Map(
    battle.teams
      .flatMap((team) => team.units)
      .map((unit) => [unit.sourceId, unit]),
  );
  const unitName = (entry) =>
    entry.name ??
    battleUnits.get(entry.unitId)?.name ??
    battleUnits.get(entry.unitId)?.typeName ??
    "Unité";
  const unitRows = result.consequences.survivors
    .filter((entry) => ownedUnitIds.has(entry.unitId))
    .map((entry) => {
      const promotion =
        entry.previousRank !== entry.rank
          ? ` · ${label(UNIT_RANKS, entry.previousRank)} → ${label(UNIT_RANKS, entry.rank)}`
          : "";
    ...
    .join("");
  const losses = result.consequences.losses.filter((entry) =>
    ownedUnitIds.has(entry.unitId),
  );
  ...
  if (result.battleLoot && bag && onCollectLoot)
    ...
    const entries = result.battleLoot.entries.filter(
      (entry) => entry.portable && (entry.allocations?.[playerId] ?? 0) > 0,
    );
    ...
    lootElement
      .querySelector("[data-collect-result-loot]")
      ?.addEventListener("click", () =>
        element.dispatchEvent(
          new CustomEvent("battle-loot-collect", { bubbles: true }),
        ),
      );
  } else lootElement.innerHTML = "<p>Aucun butin attribué.</p>";
  element
    .querySelector("[data-return-map]")
    .addEventListener("click", onReturnToMap);
}
