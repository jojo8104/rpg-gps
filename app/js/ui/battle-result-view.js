...
export function renderBattleResultView({ element, battle, result, playerId, playerTeamId, onReturnToMap }) {
  ...
  element.innerHTML = `<section class="battle-result ${victory ? "is-victory" : "is-defeat"}"><header class="battle-result__header"><p class="eyebrow">Résultat du combat</p><h3>${victory ? "Victoire" : battle.winnerTeamId === null ? "Issue indécise" : "Défaite"}</h3></header><div class="battle-result__panels"><section><h4>Héros</h4><ul>${heroRows || "<li>Aucune progression</li>"}</ul></section><section><h4>Unités survivantes</h4><ul>${unitRows || "<li>Aucune unité survivante</li>"}</ul>${lossRows ? `<h4>Unités perdues</h4><ul>${lossRows}</ul>` : ""}</section><section><h4>Butin potentiel</h4><ul>${lootRows || "<li>Aucun butin signalé</li>"}</ul><p>Le butin doit être recherché et récupéré physiquement sur le champ de bataille.</p></section></div><footer class="battle-result__actions"><button type="button" data-return-map>Retourner sur la carte</button></footer></section>`;
