/**
 * Projection DOM d'une bataille temps réel.
 * Les positions et animations sont purement visuelles ; les dégâts, ordres et
 * déplacements restent calculés par BattleEngine.
 */
import { renderUnitTypeIcon } from "./unit-icon.js";

const previousPositions = new Map();
const LANE_PERSPECTIVE = Object.freeze([0.68, 0.84, 1]);

// Conversion de la progression abstraite du moteur en coordonnées d'écran.
export function battleEntityPosition(
  entity,
  team,
  playerTeamId,
  opposingUnits = [],
  ...
  const rawX =
    (allied ? 8 + entity.progress * 84 : 92 - entity.progress * 84) + offset;
  const opponent =
    opposingUnits.find(
      (unit) => unit.id === entity.targetId || unit.targetId === entity.id,
    ) ??
    opposingUnits
      .filter((unit) => unit.lane === entity.lane && unit.state === "active")
      .sort((first, second) => second.progress - first.progress)[0];
  if (!opponent) return { x: Math.max(6, Math.min(94, rawX)), y: 50 };
  const opponentRawX = allied
    ? 92 - opponent.progress * 84
    : 8 + opponent.progress * 84;
...
export function battleEntityVisualPosition(
  entity,
  team,
  playerTeamId,
  opposingUnits = [],
  ...
  const logical = battleEntityPosition(
    entity,
    team,
    playerTeamId,
    opposingUnits,
  ...
  const y = entity.kind === "unit" && entity.lane === 2 ? 38 : logical.y;
  return { x: 50 + (logical.x - 50) * depth, y, scale: depth };
}

// Formations de sprites : le nombre rendu est borné sans modifier l'effectif réel.
function infantryFormation(type, allied, count) {
  ...
  let soldierIndex = 0;
  const ranks = militiaFormationRows(visibleCount)
    .map(
      (rank) =>
        `<b class="militia-rank" data-rank-size="${rank}">${Array.from({ length: rank }, () => `<i style="--soldier:${soldierIndex++}"></i>`).join("")}</b>`,
    )
    .join("");
...
export function militiaFormationRows(count) {
  const visibleCount = Math.min(
    24,
    Math.max(0, Number.isInteger(count) ? count : 0),
  );
  const patterns = {
    0: [],
    1: [1],
...
export function formationProjectileOffsets(count) {
  ...
  return rows.flatMap((rowSize, rowIndex) =>
    Array.from({ length: rowSize }, (_, columnIndex) => ({
      x: (columnIndex - (rowSize - 1) / 2) * 0.85,
      y: (rowIndex - (rows.length - 1) / 2) * 0.62,
    })),
  );
}
...
export function livingSoldierHealth(unit) {
  ...
  const values = Array.isArray(unit.soldierHealth)
    ? unit.soldierHealth
    : Array(unit.quantity ?? 0).fill(fallbackHealth);
  return values.filter((health) => health > 0);
}

function soldierFormation(unit, allied) {
  const maximum = Math.max(1, unit.healthPerSoldier ?? 1);
  const threshold = unit.combatHealthThreshold ?? 0;
  return `<span class="soldier-formation ${allied ? "is-allied" : "is-enemy"}" data-soldiers="${livingSoldierHealth(unit).length}" aria-hidden="true">${livingSoldierHealth(
    unit,
  )
    .map(
      (health, index) =>
        `<i class="soldier-piece ${health <= threshold ? "is-wounded" : ""}" style="--soldier:${index}"><em style="width:${Math.round(Math.min(1, health / maximum) * 100)}%"></em></i>`,
    )
    .join("")}</span>`;
}
...
export function renderBattleView({
  ...
  const allEntities = battle.teams.flatMap((team) =>
    [...team.heroes, ...team.units].map((entity) => ({ entity, team })),
  );
  const findEntity = (id) =>
  ...
  const entityIcon = (entity) => {
    ...
    return spriteTypes.has(entity.typeId)
      ? infantryFormation(
          entity.typeId,
          allied,
          entity.combatantCount ?? entity.quantity ?? 0,
        )
      : soldierFormation(entity, allied);
  };
  const latestAttacks = findLatestAttacks(battle);
  const latestAttack = latestAttacks[0] ?? null;
  const latestAttackAgeMs = latestAttack
    ? Math.max(0, battle.state.elapsedMs - latestAttack.elapsedMs)
    : Infinity;
  const commanders = player.heroes.filter((hero) => hero.state === "active");
  const powerTargetIds = new Set(
    selectedPower
      ? battle.getSpecialPowerTargets({
          teamId: playerTeamId,
          userId: selectedPower.userId,
          powerId: selectedPower.powerId,
        })
      : [],
  );
  const totalCommand = commanders.reduce(
    (sum, hero) => sum + hero.commandPoints,
    0,
  );
  ...
  const positionFor = (entity, team, index = 0) => {
    const opposingUnits =
      battle.teams.find((item) => item.id !== team.id)?.units ?? [];
    return battleEntityVisualPosition(
      entity,
      team,
      playerTeamId,
      opposingUnits,
  ...
  const fieldToken = (entity, team, index) => {
    const allied = team.id === playerTeamId;
    const position = positionFor(entity, team, index);
    const attacking =
      latestAttack?.attackerId === entity.id && latestAttackAgeMs < 900;
    const reloading =
      ["archer", "mounted-archer"].includes(
        String(entity.typeId).toLowerCase(),
      ) &&
      entity.attackCooldownMs > 0 &&
      !attacking;
    const targeted = latestAttack?.targetId === entity.id;
    const powerTarget = powerTargetIds.has(entity.id);
    const activeEffects = entity.activeEffects?.length ?? 0;
    return `<article class="unit-token field-token ${allied ? "is-ally" : "is-enemy"} ${entity.kind === "hero" ? "is-hero" : ""} ${entity.range > 1 ? "is-ranged" : "is-melee"} ${attacking ? "is-attacking" : ""} ${reloading ? "is-reloading" : ""} ${targeted ? "is-targeted" : ""} ${powerTarget ? "is-power-target" : ""} ${activeEffects > 0 ? "has-active-effect" : ""}" style="left:${position.x}%;top:${position.y}%;--perspective-scale:${position.scale};--attack-age:-${Math.min(900, latestAttackAgeMs)}ms" data-field-entity="${entity.id}" ${powerTarget ? `data-power-target="${entity.id}"` : ""} data-x="${position.x}" data-y="${position.y}" title="${entity.kind === "hero" ? "Héros immobile" : `${entity.name ?? entity.typeName ?? "Unité"} · ${entity.combatantCount} combattants · ${entity.woundedCount} blessés · ${Math.max(0, entity.maxQuantity - entity.combatantCount - entity.woundedCount)} morts ou places libres`}${activeEffects ? ` · ${activeEffects} effet(s) actif(s)` : ""}">${entityIcon(entity)}<small>${entity.kind === "hero" ? entity.health : entity.combatantCount}</small>${activeEffects ? `<mark class="effect-marker">✦${activeEffects}</mark>` : ""}${entity.kind === "unit" ? healthBar(entity) : ""}</article>`;
  };
  ...
  const lane = (index) => {
    ...
    const playerUnits = player.units.filter(
      (unit) => unit.lane === index && unit.state === "active",
    );
    const canRetreat =
      playerUnits.some((unit) => !unit.retreating) &&
      battle.status === "active" &&
      totalCommand >= retreatCost;
  ...
  const hand = player.units.filter(
    (unit) => unit.lane === null && unit.state === "active",
  );
  const handCard = (unit) =>
    `<button type="button" class="unit-token hand-token is-ally ${unit.id === selectedUnitId ? "is-selected" : ""} ${powerTargetIds.has(unit.id) ? "is-power-target" : ""}" draggable="${selectedPower ? "false" : "true"}" data-unit="${unit.id}" ${powerTargetIds.has(unit.id) ? `data-power-target="${unit.id}"` : ""} aria-label="Sélectionner ${unit.name ?? unit.typeName ?? "une unité"}, ${unit.combatantCount} combattants"><span class="hand-unit-icon">${renderUnitTypeIcon(unit)}</span><strong>${unit.name ?? unit.typeName ?? unit.typeId}</strong><small>${unit.combatantCount} soldats</small>${healthBar(unit)}</button>`;
  const attackVisual = createAttackVisual(
    latestAttacks,
    findEntity,
    positionFor,
  ...
  const powers = commanders
    .flatMap((hero) =>
      hero.specialPowerIds.map((powerId) => powerButton(hero, powerId, 1)),
    )
    .concat(
      player.units
        .filter((unit) => unit.state === "active")
        .flatMap((unit) =>
          unit.specialPowerIds.map((powerId) => powerButton(unit, powerId, 1)),
        ),
    )
    .join("");
  ...
  const countdown =
    battle.status === "countdown"
      ...
      : "";
  const targeting = selectedPower
    ? `<div class="power-targeting"><strong>${selectedPower.name}</strong><span>Touchez une cible lumineuse.</span><button type="button" data-cancel-power>Annuler</button></div>`
    : "";
  ...
  element.querySelectorAll("[data-unit]").forEach((item) => {
    ...
    item.addEventListener("dragstart", (event) => {
      ...
      event.dataTransfer.setData("text/plain", item.dataset.unit);
      onDragState(true);
    });
    item.addEventListener("dragend", () => onDragState(false));
    item.addEventListener("click", () => {
      if (!powerTargetIds.has(item.dataset.unit))
        onSelectUnit(item.dataset.unit);
    });
  ...
  element.querySelectorAll("[data-line]").forEach((target) => {
    ...
    target.addEventListener("drop", (event) => {
      event.preventDefault();
      target.classList.remove("is-drop-target");
      const unitId = event.dataTransfer.getData("text/plain") || selectedUnitId;
      onDragState(false);
      if (unitId) onAssign(unitId, Number(target.dataset.line));
    });
    target.addEventListener("click", (event) => {
      if (
        selectedUnitId &&
        !event.target.closest(
          "[data-unit], [data-retreat-line], [data-power-target]",
        )
      )
        onAssign(selectedUnitId, Number(target.dataset.line));
    });
  });
  element
    .querySelectorAll("[data-retreat-line]")
    .forEach((button) =>
      button.addEventListener("click", () =>
        onRetreatLine(Number(button.dataset.retreatLine)),
      ),
    );
  element.querySelectorAll("[data-power-user]").forEach((button) =>
    button.addEventListener("click", () => {
      const definition = battle.getSpecialPowerDefinition(
        button.dataset.powerId,
      );
      const target = definition?.activation?.target ?? "none";
      const payload = {
        userId: button.dataset.powerUser,
        powerId: button.dataset.powerId,
        cost: Number(button.dataset.powerCost),
        name: definition?.name ?? button.dataset.powerId,
      };
      if (target === "none" || target === "self")
        onActivatePower({
          ...payload,
          targetId: target === "self" ? payload.userId : null,
        });
      else onSelectPower(payload);
    }),
  );
  element.querySelectorAll("[data-power-target]").forEach((target) =>
    target.addEventListener("click", (event) => {
      event.stopPropagation();
      onActivatePower({
        ...selectedPower,
        targetId: target.dataset.powerTarget,
      });
    }),
  );
  element
    .querySelector("[data-cancel-power]")
    ?.addEventListener("click", onCancelPower);
  element.querySelector("[data-flee]").addEventListener("click", onFlee);
  element
    .querySelector("[data-surrender]")
  ...
  function powerButton(user, powerId, cost) {
    const commander =
      user.kind === "hero"
        ? user
        : commanders.find((hero) => hero.playerId === user.playerId);
    const definition = battle.getSpecialPowerDefinition(powerId);
    const actualCost = definition?.activation?.cost ?? cost;
    const rank = user.aptitudeRanks?.[powerId];
    const disabled =
      battle.status !== "active" ||
      !commander ||
      commander.commandPoints < actualCost;
...
// Adaptateur Pointer Events pour transmettre une intention d'affectation de ligne.
function enablePointerDrag(item, element, onAssign, onDragState) {
  ...
  const finish = (event) => {
    ...
    target?.classList.remove("is-drop-target");
    onDragState(false);
    if (moved && target && element.contains(target))
      onAssign(item.dataset.unit, Number(target.dataset.line));
    else item.click();
    target = null;
  };
...
// Effets transitoires dérivés du journal du moteur, sans état métier local.
function findLatestAttacks(battle) {
  ...
  for (let index = battle.eventLog.length - 1; index >= 0; index -= 1) {
    ...
    if (
      !["attack", "breakthrough_attack", "retreat_attack"].includes(event.type)
    )
      continue;
...
function createAttackVisual(events, findEntity, positionFor, elapsedMs) {
  ...
  const lines = events
    .map((event) => {
      const attacker = findEntity(event.attackerId);
      const target = findEntity(event.targetId);
      if (!attacker || !target) return "";
      const attackerPosition = positionFor(attacker.entity, attacker.team);
      const targetPosition = positionFor(target.entity, target.team);
      const attackerX = attackerPosition.x;
      const targetX = targetPosition.x;
      const attackerY =
        ((attacker.entity.lane ?? 1) + attackerPosition.y / 100) * (100 / 3);
      const targetY =
        ((target.entity.lane ?? 1) + targetPosition.y / 100) * (100 / 3);
      if (attacker.entity.range <= 1) return "";
      if (String(attacker.entity.typeId).toLowerCase() === "archer") {
        const flightAgeMs = Math.min(
          1_800,
          Math.max(0, elapsedMs - event.elapsedMs),
        );
        return formationProjectileOffsets(
          attacker.entity.combatantCount ?? attacker.entity.quantity ?? 0,
        )
          .map((offset) => {
            const startX = attackerX + offset.x;
            const startY = attackerY + offset.y;
            const endX = targetX + offset.x;
            const endY = targetY + offset.y;
            const direction = endX >= startX ? 1 : -1;
            const tipX = startX + direction * 2.3;
            const tailX = startX - direction * 2.3;
            return `<g class="arrow-projectile" style="--flight-age:-${flightAgeMs}ms"><ellipse class="arrow-ground-shadow" cx="${startX}" cy="${startY + 1.5}" rx="1.2" ry=".28"></ellipse><line class="arrow-shaft" x1="${tailX}" y1="${startY}" x2="${tipX}" y2="${startY}"></line><path class="arrow-head" d="M ${tipX} ${startY} l ${-direction * 0.85} -0.55 l 0 1.1 Z"></path><animateTransform attributeName="transform" type="translate" from="0 0" to="${endX - startX} ${endY - startY}" dur="1.8s" begin="-${flightAgeMs / 1_000}s" fill="freeze"></animateTransform></g>`;
          })
          .join("");
      }
  ...
  events.forEach((event) => {
    const total = damageByTarget.get(event.targetId) ?? {
      damage: 0,
      losses: 0,
    };
    total.damage += event.damage ?? 0;
    total.losses += event.losses ?? 0;
    damageByTarget.set(event.targetId, total);
  });
  const effects = [...damageByTarget]
    .map(([targetId, total]) => {
      ...
      const value =
        target.entity.kind === "hero"
          ? `-${total.damage} PV`
          : `-${total.damage} PV${total.losses > 0 ? ` · ${total.losses} mort${total.losses > 1 ? "s" : ""}` : ""}`;
...
function animateMovement(element) {
  ...
  element.querySelectorAll("[data-field-entity]").forEach((token) => {
    const id = token.dataset.fieldEntity;
    const current = { x: Number(token.dataset.x), y: Number(token.dataset.y) };
    ...
    if (previous && (previous.x !== current.x || previous.y !== current.y)) {
      ...
      setTimeout(() => token.classList.remove("is-moving"), 480);
    }
    previousPositions.set(id, current);
  });
  for (const id of previousPositions.keys())
    if (!present.has(id)) previousPositions.delete(id);
}
