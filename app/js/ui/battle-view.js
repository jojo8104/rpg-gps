...
export function renderBattleView({ element, battle, playerTeamId, message, selectedUnitId = null, selectedPower = null, onSelectUnit, onAssign, onDragState, onRetreatLine, onSelectPower, onCancelPower, onActivatePower, onFlee, onSurrender }) {
  ...
  const heroHud = (team, side) => `<div class="battle-hero-hud is-${side}">${team.heroes.map((hero) => `<span class="hero-health ${hero.state !== "active" ? "is-out" : ""}"><b>${hero.name ?? "H"}</b><i role="progressbar" aria-label="${hero.health} PV sur ${hero.maxHealth}" aria-valuemin="0" aria-valuemax="${hero.maxHealth}" aria-valuenow="${hero.health}"><em style="width:${Math.round(hero.health / hero.maxHealth * 100)}%"></em><small>PV ${hero.health}/${hero.maxHealth}</small></i></span>`).join("")}</div>`;
