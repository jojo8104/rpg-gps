...
export function bindEquipmentView(element, { onEquip, onUnequip }) {
  ...
  element.querySelectorAll("[data-equipment-slot]").forEach((button) => button.addEventListener("click", () => {
    const slotId = button.dataset.equipmentSlot;
    const choices = JSON.parse(decodeURIComponent(button.dataset.equipmentChoices));
    ...
    menu.hidden = false;
    menu.querySelector(".equipment-menu-close").addEventListener("click", () => { menu.hidden = true; });
    menu.querySelectorAll("[data-equip-package]").forEach((choice) => choice.addEventListener("click", () => onEquip(choice.dataset.equipPackage, slotId)));
...
export function equipmentSlotState(hero, slot) {
  ...
  const available = hero.carriedLoot.map((entry) => ({ ...entry, definition: getItemDefinition(entry.itemId) })).filter((entry) => entry.definition?.category === "equipment" && isCompatible(entry.definition, slot.id));
  const betterAvailable = definition !== null && available.some((entry) => equipmentScore(entry.definition) > equipmentScore(definition));
  const className = definition ? `is-equipped ${betterAvailable ? "has-upgrade" : "is-best"}` : available.length ? "has-available" : "is-empty";
  return { definition, available, betterAvailable, className };
}

function equipmentChoice(choice) { return `<button type="button" class="equipment-menu-choice" data-equip-package="${choice.id}"><span>${choice.icon}</span><strong>${choice.name}</strong><small>${choice.modifiers}</small></button>`; }
function isCompatible(definition, slotId) { return definition.equipmentSlot === slotId || definition.equipmentSlot === "accessory" && slotId.startsWith("accessory"); }
function equipmentScore(definition) { return Object.values(definition.modifiers ?? {}).reduce((sum, value) => sum + Number(value || 0), 0); }
