...
function mappedLocations({ knownOnly = true } = {}) {
  ...
  return game.locations.filter((location) => location.state !== "destroyed" && (!knownOnly || player.knowsLocation(location.id))).map((location) => {
    ...
    if (can("recruit")) location.recruitment.availableUnitTypeIds.forEach((type) => actions.push({ id: `recruit:${type}`, label: `Recruter ${type}` }));
    ...
    if (can("deposit")) { Object.entries(hero.resources).filter(([, amount]) => amount > 0).forEach(([id, amount]) => actions.push({ id: `deposit-resource:${id}`, label: `Déposer ${Math.floor(amount)} ${id}` })); hero.carriedLoot.forEach((item) => actions.push({ id: `deposit-item:${item.id}`, label: `Déposer ${item.quantity} ${item.itemId}` })); }
...
function openBattle() {
  ...
  if (sourceLocationId && !game.canPerformLocationAction({ playerId: "local", locationId: sourceLocationId, action: "attack" })) { locationMessage = "Attaque interdite par la relation avec ce lieu."; return selectLocation(sourceLocationId); }
  ...
  activeBattle = game.createBattle({ teamParticipants: [{ id: "heroes", heroIds: [hero.id] }, { id: "bandits", heroIds: defenderHeroIds, locationId: battleLocationId }], position: asGps(heroPosition), sourceLocationId: battleLocationId, sourceEnemyTeamId: "bandits", loot: [{ id: "bandit-gold", itemId: "gold", quantity: 12, portable: true, weightPerUnit: .1, valuePerUnit: 1 }, { id: "bandit-barricade", itemId: "barricade", quantity: 1, portable: false, weightPerUnit: 80, valuePerUnit: 25 }] });
  activeBattle.teams[0].units.forEach((unit) => { unit.lane = null; unit.progress = 0; }); activeBattle.teams[1].units.forEach((unit, index) => { unit.lane = index % 3; unit.progress = 0; }); battleResolved = false; battleResult = null; battleMessage = "Placez vos unités sur une ligne."; setBattleNavigationLocked(true); switchView("battle"); renderBattle(); logTest("Combat déclenché à la position du joueur.");
