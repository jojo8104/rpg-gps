...
let data, game, hero, enemyHero, mapView, gpsTracker, locationEngine, interactionEngine, activeBattle, battleTimer;
...
function selectLocation(id) { currentLocationId = id; renderLocationSheet({ element: ui.sheet, location: mappedLocations().find((item) => item.id === id), message: locationMessage, onClose: () => closeSheet(ui.sheet), onAction: runAction }); }
...
$("#validate-area").onclick = () => { try { validatedPlayArea = field.createPlayArea(); game.setup.playArea = validatedPlayArea; playAreaGrid = new PlayAreaGrid({ playArea: validatedPlayArea, cellSizeMeters: mode === "gps" ? 100 : 100_000 }); interactionMode = null; lastVisitedCellId = null; $("#toggle-heatmap").disabled = false; mapView.setPlayArea(validatedPlayArea.polygon); logTest(`Zone validée · ${(validatedPlayArea.getAreaSquareMeters() / 10_000).toFixed(1)} ha · ${playAreaGrid.cells.length} cellules.`); applyPosition(heroPosition); } catch (error) { logTest(error.message); } };
