...
export class Unit {
  constructor({ id, ownerPlayerId, typeId, quantity, name = null, number = null, rank = "soldier", level = 1, experience = 0, equipmentIds = [], specialPowerIds = [], state = "active" }) {
    ...
    this.name = name === null ? null : Unit.#text(name, "Le nom de l'unité");
    this.number = number === null ? null : Unit.#positiveInteger(number, "Le numéro de l'unité");
  ...
  toJSON() { return { id: this.id, ownerPlayerId: this.ownerPlayerId, typeId: this.typeId, name: this.name, number: this.number, quantity: this.quantity, maxQuantity: this.maxQuantity, rank: this.rank, level: this.level, experience: this.experience, equipmentIds: [...this.equipmentIds], specialPowerIds: [...this.specialPowerIds], state: this.state }; }
