export class AutonomousGroupTrace {
  constructor({
    ...
    this.soldierCount = nonNegativeInteger(
      soldierCount,
      "Le nombre de soldats",
    );
    this.occupiedCargoSlots = nonNegativeInteger(
      occupiedCargoSlots,
    ...
    if (!Number.isFinite(createdAt))
      throw new TypeError("L'heure de création de la trace est invalide.");
    if (!Number.isFinite(decayPerMinute) || decayPerMinute <= 0)
      throw new RangeError("La décroissance doit être strictement positive.");
  ...
  getScore(at) {
    return Math.max(
      0,
      this.initialDetectionScore -
        (this.decayPerMinute * Math.max(0, at - this.createdAt)) / 60_000,
    );
  }
  getPerceivedScore({
    at,
    ...
    if (!Number.isFinite(distance) || distance < 0)
      throw new RangeError(
        "La distance de lecture doit être positive ou nulle.",
      );
    if (!Number.isFinite(distancePerPoint) || distancePerPoint <= 0)
      throw new RangeError(
        "La portée par point doit être strictement positive.",
      );
    if (!Number.isFinite(detectionBonus) || detectionBonus < 0)
      throw new RangeError("Le bonus de détection doit être positif ou nul.");
    if (!Number.isFinite(detectionMultiplier) || detectionMultiplier <= 0)
      throw new RangeError(
        "Le multiplicateur de détection doit être strictement positif.",
      );
    return Math.max(
      0,
      this.getScore(at) * detectionMultiplier +
        detectionBonus -
  ...
  read({ at, minimumScore = 5, analysisBonus = 0 }) {
    ...
    const level = Math.max(
      1,
      Math.min(5, Math.floor((score + analysisBonus) / 3) + 1),
    );
    return {
      traceId: this.id,
      level,
      ...
      importance:
        level >= 2
          ? approximateImportance(this.soldierCount, this.occupiedCargoSlots)
          : null,
...
function approximateImportance(soldiers, slots) {
  const value = soldiers + slots * 10;
  return value >= 80
    ? "very_large"
    : value >= 40
      ? "large"
