...
export class ResultEvaluationService {
  evaluate({ metrics, thresholds = { gold: 0.85, silver: 0.6, bronze: 0.3 } }) {
    if (!Array.isArray(metrics) || metrics.length === 0)
      throw new TypeError("Au moins une métrique est requise.");
    let weighted = 0;
    let weights = 0;
    const details = metrics.map(({ id, value, target, weight = 1 }) => {
      if (
        ...
        throw new TypeError("Une métrique d'évaluation est invalide.");
    ...
    const grade =
      ratio >= thresholds.gold
        ? "gold"
        : ratio >= thresholds.silver
          ? "silver"
          : ratio >= thresholds.bronze
            ? "bronze"
            : "failed";
