...
export class HeroArmyModifier {
  static calculate({
    ...
    if (!hero || !Array.isArray(units) || !(unitDefinitions instanceof Map)) {
      throw new TypeError(
        "Le héros, ses unités et leurs définitions sont requis.",
      );
    }
    if (!new Set(["casual", "expert"]).has(moraleMode))
    ...
    return {
      attackBonus:
        hero.finalStats?.attack ?? hero.commandStats?.attackBonus ?? 0,
      defenseBonus:
        hero.finalStats?.defense ?? hero.commandStats?.defenseBonus ?? 0,
      moraleBonus:
        (hero.finalStats?.morale ?? hero.commandStats?.moraleBonus ?? 0) +
        morale.total,
      speedMultiplier:
        speed.multiplier * ((hero.finalStats?.mobility ?? 3) / 3),
  ...
  static #morale(hero, mode, context) {
    const allowed =
      mode === "expert" ? EXPERT_MORALE_SOURCES : CASUAL_MORALE_SOURCES;
    const factors = [
      ...(hero.moraleHistory ?? []),
      ...(context.moraleFactors ?? []),
    ...
    return {
      total: details.reduce((sum, factor) => sum + factor.value, 0),
      details,
    };
  }
}
