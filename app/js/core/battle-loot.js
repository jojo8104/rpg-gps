...
export class BattleLoot {
  constructor({ id, battleId, entries, shares, now = () => Date.now() }) {
    ...
    this.entries = entries.map((entry) => structuredClone({ id: entry.id, itemId: entry.itemId, quantity: entry.quantity, portable: entry.portable, valuePerUnit: entry.valuePerUnit, allocations: entry.allocations ?? {} }));
  ...
  collect({ playerId, bag, selection }) {
    ...
    for (const entry of this.entries) {
      ...
      const requiredSlots = Math.max(0, Math.ceil((currentQuantity + requestedQuantity) / bundleSize) - currentSlots);
      remainingSlots -= requiredSlots; inventory.quantitiesByItem[entry.itemId] = currentQuantity + requestedQuantity; inventory.slotsByItem[entry.itemId] = currentSlots + requiredSlots;
      planned.push({ entry, itemId: entry.itemId, quantity: requestedQuantity, valuePerUnit: entry.valuePerUnit });
    }
    ...
    this.collectionLog.push({ playerId, collected: structuredClone(collected), at: this.now() });
    if (this.entries.every((entry) => !entry.portable || entry.quantity === 0 || Object.values(entry.allocations).every((quantity) => quantity === 0))) this.status = "COLLECTED";
    return { success: true, collected, remainingSlots };
  }

  toJSON() { return { id: this.id, battleId: this.battleId, entries: structuredClone(this.entries), shares: structuredClone(this.shares), status: this.status, collectionLog: structuredClone(this.collectionLog) }; }
}
