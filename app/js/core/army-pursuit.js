...
export function armySpeed(units, speedFor = (unit) => unit.speed) {
  ...
  const survivors = units.filter(
    (unit) => unit.state !== "defeated" && (unit.quantity ?? 0) > 0,
  );
  const soldiers = survivors.reduce((total, unit) => total + unit.quantity, 0);
  if (soldiers === 0) return 0;
  return (
    survivors.reduce(
      (total, unit) => total + speedFor(unit) * unit.quantity,
      0,
    ) / soldiers
  );
}
...
export function pursuitRounds(winnerSpeed, loserSpeed) {
  if (
    !Number.isFinite(winnerSpeed) ||
    !Number.isFinite(loserSpeed) ||
    winnerSpeed < 0 ||
    loserSpeed < 0
  )
    throw new RangeError(
      "Les vitesses d'armée doivent être positives ou nulles.",
    );
