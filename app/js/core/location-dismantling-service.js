...
export class LocationDismantlingService {
  ...
  start(location, structureId, at) {
    ...
    if (
      location.dismantlings.some((entry) => entry.structureId === structureId)
    )
      return { success: false, reason: "already_dismantling" };
    const task = {
      structureId,
      level,
      deadline: this.deadlineService.create({
        id: `dismantle:${location.id}:${structureId}`,
        durationMs: this.durationMs * level,
        startedAt: at,
      }),
    };
    location.dismantlings.push(task);
  ...
  completeReady(location, at) {
    ...
    location.dismantlings = location.dismantlings.filter((task) => {
      if (!this.deadlineService.evaluate(task.deadline, at).expired)
        return true;
      const current = location.infrastructure[task.structureId] ?? 0;
      if (current <= 0) return false;
      delete location.infrastructure[task.structureId];
      const recovered = Object.fromEntries(
        Object.entries(this.yields).map(([resource, amount]) => [
          resource,
          location.depositResource(resource, amount * task.level),
        ]),
      );
      completed.push({
