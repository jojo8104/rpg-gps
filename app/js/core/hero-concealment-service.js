...
export class HeroConcealmentService {
  constructor({ stationaryDurationMs = 20_000, movementThresholdMeters = 5, concealedSignatureMultiplier = .6, distanceFn = distanceMeters } = {}) {
  ...
  update({ position, accuracy = 0, at = Date.now() }) {
    if (!this.lastPosition) { this.lastPosition = { ...position, accuracy }; this.stationarySince = at; return { moved: false, concealmentCancelled: false }; }
    const rawDistance = this.distanceFn(this.lastPosition, position);
    const uncertainty = Math.min(accuracy || 0, this.lastPosition.accuracy || 0);
    const moved = Math.max(0, rawDistance - uncertainty) > this.movementThresholdMeters;
    const concealmentCancelled = moved && this.concealed;
    if (moved) { this.stationarySince = at; this.concealed = false; }
