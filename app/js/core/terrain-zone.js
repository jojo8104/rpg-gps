...
export class TerrainZone {
  constructor({
    ...
    if (!TERRAIN_TYPES.has(type))
      throw new RangeError("Le type de terrain est invalide.");
    if (!CERTAINTY_LEVELS.has(certainty))
      throw new RangeError("Le niveau de certitude est invalide.");
    this.type = type;
    ...
    this.source =
      source === null
        ? null
        : TerrainZone.#requireText(source, "La source du terrain");
    this.discoveredBy =
      discoveredBy === null
        ? null
        : TerrainZone.#requireText(discoveredBy, "Le découvreur");
  ...
  static #createCells(cells) {
    if (!Array.isArray(cells) || cells.length === 0)
      throw new RangeError(
        "Une zone de terrain doit contenir au moins une case.",
      );
    ...
    return cells.map((cell) => {
      if (
        ...
        throw new RangeError("Une case de terrain est invalide.");
      const key = `${cell.x}:${cell.y}`;
      if (uniqueCells.has(key))
        throw new RangeError(
          "Une zone ne peut pas contenir deux fois la même case.",
        );
      uniqueCells.add(key);
  ...
  static #createModifiers(modifiers) {
    if (
      ...
      throw new TypeError(
        "Les modificateurs de terrain doivent être un objet.",
      );
    return Object.fromEntries(
      Object.entries(modifiers).map(([name, value]) => {
        if (!Number.isFinite(value) || value <= 0)
          throw new RangeError("Un modificateur de terrain doit être positif.");
        return [
          TerrainZone.#requireText(name, "Le nom de modificateur"),
          value,
        ];
      }),
    );
