export function describeHeroStat(stat, value) {
  ...
  if (stat === "speed") return value < 2 ? "lent" : value < 4 ? "normal" : "rapide";
  return value < 4 ? "incompétent" : value < 7 ? "mauvais" : value < 11 ? "normal" : value < 15 ? "bon" : "compétent";
}

export function buildLocationIntel({ location, snapshot, knowledgeLevel, owner = null, description = "", heroes = [] }) {
  ...
  const presences = heroes.map((hero) => {
    ...
    const values = { attack: 10 + hero.level * 2, defense: 5 + hero.level, speed: 2, command: 2 + hero.level };
    const stats = Object.entries(values).map(([id, value]) => ({ id, assessment: describeHeroStat(id, value), value: knowledgeLevel >= 3 ? value : null }));
    const army = knowledgeLevel >= 3 ? (hero.army?.units ?? []).map((unit) => ({ type: unit.name ?? unit.typeId, quantity: unit.quantity })) : [];
    return { id: hero.id, label: knowledgeLevel >= 3 ? hero.name : `Héros ${hero.className ?? hero.classId ?? "inconnu"} présent`, className: hero.className ?? hero.classId ?? "inconnu", stats, army };
  });
  return {
    ...snapshot,
    knowledgeLevel,
