...
export class DeadlineService {
  create({ id, durationMs, startedAt }) {
    if (typeof id !== "string" || !id.trim())
      throw new TypeError("L'identifiant de l'échéance est requis.");
    if (
      ...
      throw new RangeError(
        "La durée et le départ de l'échéance sont invalides.",
      );
  ...
  evaluate(deadline, at) {
    if (
      ...
      throw new TypeError("L'échéance ou l'heure est invalide.");
    ...
    return {
      ...
      progress: Math.max(
        ...
        Math.min(
          1,
          (at - deadline.startedAt) / (deadline.expiresAt - deadline.startedAt),
        ),
      ),
    };
  }
