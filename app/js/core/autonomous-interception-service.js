...
export class AutonomousInterceptionService {
  constructor({ engagementRadiusMeters = 8, minimumReactionMs = 15_000 } = {}) {
    if (!Number.isFinite(engagementRadiusMeters) || engagementRadiusMeters <= 0)
      throw new RangeError(
        "Le rayon d'interception doit être strictement positif.",
      );
    if (!Number.isFinite(minimumReactionMs) || minimumReactionMs <= 0)
      throw new RangeError(
        "Le délai de réaction doit être strictement positif.",
      );
  ...
  detect(segment, targets, { observerDetectionMultiplier = 1 } = {}) {
    ...
    const candidates = targets
      ...
      .map((target) => {
        ...
        return {
          target,
          ...closest,
          distance: distanceMeters(closest.position, target.position),
        };
      })
      .filter(
        (candidate) =>
          candidate.distance <=
          this.engagementRadiusMeters *
            positiveMultiplier(observerDetectionMultiplier) *
            positiveMultiplier(candidate.target.concealmentMultiplier),
      )
      .sort(
        (first, second) =>
          first.fraction - second.fraction ||
          String(first.target.id).localeCompare(String(second.target.id)),
      );
    ...
    return {
      ...
      occurredAt:
        segment.fromAt + (segment.toAt - segment.fromAt) * match.fraction,
      distanceMeters: match.distance,
    };
  }

  create(group, detection, { id, movementService }) {
    const remaining = movementService.interrupt(group, {
      position: detection.position,
      occurredAt: detection.occurredAt,
    });
    const aggressive = group.behavior === "aggressive";
    const reactionDurationMs = aggressive
      ? 0
      : Math.max(
          this.minimumReactionMs,
          Math.ceil(
            (this.engagementRadiusMeters /
              Math.max(0.1, remaining.speedMetersPerSecond)) *
              1000,
          ),
        );
    group.interruption = {
      ...
      target: {
        kind: detection.target.kind ?? "hero",
        id: detection.target.id,
      },
      position: { ...detection.position },
      startedAt: detection.occurredAt,
      reactionDeadlineAt: detection.occurredAt + reactionDurationMs,
      mode: aggressive ? "immediate_attack" : "reaction_window",
      status: aggressive ? "attacking" : "pending",
      resume: {
        destination: remaining.destination,
        speedMetersPerSecond: remaining.speedMetersPerSecond,
      },
    };
  ...
  letEscape(group, { now, movementService }) {
    if (
      group.interruption?.status !== "pending" ||
      now < group.interruption.reactionDeadlineAt
    )
      return false;
  ...
  resolve(group, { now, action = "intercept" } = {}) {
    ...
    if (
      !interruption ||
      !["pending", "attacking"].includes(interruption.status)
    )
      return { success: false, reason: "no_active_interception" };
    if (
      interruption.status === "pending" &&
      now > interruption.reactionDeadlineAt
    )
      return { success: false, reason: "reaction_expired" };
    if (action !== "intercept" && action !== "attack")
      return { success: false, reason: "invalid_action" };
    const information =
      group.type === "messenger"
        ? structuredClone(group.message)
        : group.type === "prospecting"
          ? structuredClone(group.mission?.report ?? null)
          : null;
    let outcome;
    ...
    const cargo =
      outcome === "plundered"
        ? group.cargo.map((entry) => structuredClone(entry))
        : [];
    ...
    return {
      ...
      outcome,
      groupId: group.id,
      target: structuredClone(interruption.target),
      information,
      cargo,
      at: now,
    };
...
function closestPoint(start, end, point) {
  ...
  const fraction =
    lengthSquared === 0
      ? 0
      : Math.max(0, Math.min(1, (px * x2 + py * y2) / lengthSquared));
  return {
    fraction,
    position: {
