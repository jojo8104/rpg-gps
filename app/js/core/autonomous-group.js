...
export const AUTONOMOUS_GROUP_TYPES = Object.freeze([
  "rogue",
  "army",
  "messenger",
  "convoy",
...
export const AUTONOMOUS_OWNER_KINDS = Object.freeze([
  "player",
  "faction",
  "location",
  "independent",
...
export class AutonomousGroup {
  constructor({
    ...
    this.type = AutonomousGroup.#choice(
      type,
      AUTONOMOUS_GROUP_TYPES,
      "Le type du groupe autonome",
    );
    ...
    this.originLocationId = AutonomousGroup.#optionalText(
      originLocationId,
      "Le lieu d'origine",
    );
    ...
    if (morale !== null && (!Number.isFinite(morale) || morale < 0))
      throw new RangeError("Le moral doit être positif, nul ou absent.");
    this.morale = morale;
    this.movement = AutonomousGroup.#optionalRecord(movement, "Le déplacement");
    this.interruption = AutonomousGroup.#optionalRecord(
      interruption,
      "L'interruption",
    );
    ...
    this.traceAnchor =
      traceAnchor === null
        ? { ...this.position }
        : AutonomousGroup.#position(traceAnchor);
    this.detectionMultiplier = AutonomousGroup.#positiveNumber(
      detectionMultiplier,
      "Le multiplicateur de détection",
    );
    this.concealmentMultiplier = AutonomousGroup.#positiveNumber(
      concealmentMultiplier,
      "Le multiplicateur de discrétion",
    );
  }
  ...
  toJSON() {
    return {
      ...
      interruption:
        this.interruption === null ? null : structuredClone(this.interruption),
  ...
  static #owner(owner) {
    if (owner === null || Array.isArray(owner) || typeof owner !== "object")
      throw new TypeError("Le propriétaire doit être un objet.");
    return {
      kind: AutonomousGroup.#choice(
        owner.kind,
        AUTONOMOUS_OWNER_KINDS,
  ...
  static #position(position) {
    if (
      ...
      throw new TypeError("La position du groupe autonome est invalide.");
  ...
  static #optionalRecord(value, label) {
    ...
    if (Array.isArray(value) || typeof value !== "object")
      throw new TypeError(`${label} doit être un objet ou null.`);
  ...
  static #records(values, label) {
    ...
    return values.map((value) => {
      if (value === null || Array.isArray(value) || typeof value !== "object")
        throw new TypeError(
          `Chaque entrée de ${label.toLowerCase()} doit être un objet.`,
        );
      return structuredClone(value);
    });
  }
  ...
  static #choice(value, choices, label) {
    ...
    if (!choices.includes(checked))
      throw new RangeError(`${label} n'est pas reconnu.`);
    return checked;
  }

  static #optionalText(value, label) {
    return value === null ? null : AutonomousGroup.#text(value, label);
  }
  static #positiveNumber(value, label) {
    if (!Number.isFinite(value) || value <= 0)
      throw new RangeError(`${label} doit être strictement positif.`);
    return value;
  }
