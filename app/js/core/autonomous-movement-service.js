...
export class AutonomousMovementService {
  start(group, { destination, speedMetersPerSecond, now }) {
    ...
    if (!Number.isFinite(speedMetersPerSecond) || speedMetersPerSecond <= 0)
      throw new RangeError(
        "La vitesse du groupe doit être strictement positive.",
      );
    if (!Number.isFinite(now))
      throw new TypeError("L'heure de départ est invalide.");
    ...
    const durationMs =
      distance === 0 ? 0 : Math.ceil((distance / speedMetersPerSecond) * 1000);
    group.movement = {
      origin,
  ...
  positionAt(movement, at) {
    ...
    const progress =
      duration <= 0
        ? 1
        : Math.max(0, Math.min(1, (at - movement.startedAt) / duration));
    return {
      latitude:
        movement.origin.latitude +
        (movement.destination.latitude - movement.origin.latitude) * progress,
      longitude:
        movement.origin.longitude +
        (movement.destination.longitude - movement.origin.longitude) * progress,
    };
  }

  advance(group, now) {
    if (group.status !== "traveling" || group.movement === null)
      return { changed: false, arrived: false, segment: null };
    if (!Number.isFinite(now))
      throw new TypeError("L'heure de mise à jour est invalide.");
  ...
  interrupt(group, { position, occurredAt }) {
    ...
    if (group.movement === null)
      throw new Error("Le groupe ne possède aucun trajet à interrompre.");
