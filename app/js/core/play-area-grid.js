...
export class PlayAreaGrid {
  constructor({ playArea, cellSizeMeters = 100, cells = null }) {
    ...
    if (!Number.isFinite(cellSizeMeters) || cellSizeMeters <= 0) throw new RangeError("La taille des cellules doit être positive.");
    ...
    this.origin = { latitude: Math.min(...latitudes), longitude: Math.min(...longitudes) };
    this.averageLatitude = latitudes.reduce((sum, value) => sum + value, 0) / latitudes.length;
    this.latitudeStep = cellSizeMeters / 111_320;
    this.longitudeStep = cellSizeMeters / (111_320 * Math.max(.01, Math.cos(this.averageLatitude * Math.PI / 180)));
    this.cells = cells === null ? this.#generate(Math.max(...latitudes), Math.max(...longitudes)) : cells.map(normalizeCell);
    this.cellByCoordinates = new Map(this.cells.map((cell) => [`${cell.row}:${cell.column}`, cell]));
  }

  getCellAt(position) {
    if (!this.playArea.contains(position)) return null;
    const row = Math.floor((position.latitude - this.origin.latitude) / this.latitudeStep);
    const column = Math.floor((position.longitude - this.origin.longitude) / this.longitudeStep);
    return this.cellByCoordinates.get(`${row}:${column}`) ?? null;
  }

  recordVisit(position, { activity = 1, visitedAt = Date.now() } = {}) {
    if (!Number.isFinite(activity) || activity < 0) throw new RangeError("L'activité doit être positive ou nulle.");
  ...
  #generate(maxLatitude, maxLongitude) {
    const rows = Math.ceil((maxLatitude - this.origin.latitude) / this.latitudeStep);
    const columns = Math.ceil((maxLongitude - this.origin.longitude) / this.longitudeStep);
    if (rows * columns > 20_000) throw new RangeError("La zone est trop grande pour cette taille de cellule.");
    const cells = [];
