...
import {
  ...
  levelForExperience,
  totalXpForLevel,
} from "./hero-progression-config.js";
...
export class Hero {
  constructor({
    ...
    this.classId =
      classId === null
        ? null
        : Hero.#requireText(classId, "L'identifiant de classe");
    this.baseStats = Hero.#createStats(
      baseStats ?? {
        attack: commandStats.attackBonus ?? 0,
        defense: commandStats.defenseBonus ?? 0,
        morale: commandStats.moraleBonus ?? 0,
        mobility: 1,
        command: maxCommandPoints,
        health: maxHealth ?? 30,
      },
      "Les statistiques initiales",
    );
    this.statGrowth = Hero.#createStats(
      statGrowth,
      "La progression des statistiques",
      true,
    );
    this.equipmentModifiers = Hero.#createStats(
      equipmentModifiers,
      "Les bonus d'équipement",
      true,
    );
    this.temporaryModifiers = Hero.#createStats(
      temporaryModifiers,
      "Les bonus temporaires",
      true,
    );
    this.maxHealth = Hero.#requirePositiveNumber(
      maxHealth ?? this.finalStats.health,
      "Les PV maximum",
    );
    this.health = Hero.#requireHealth(health ?? this.maxHealth, this.maxHealth);
    this.state = Hero.#requireText(state, "L'état du héros");
    this.pursuitCooldownUntil =
      pursuitCooldownUntil === null
        ? null
        : Hero.#requireNonNegativeNumber(
            pursuitCooldownUntil,
            "La fin du cooldown",
          );
    ...
    this.commandRank = HERO_GRADES.some((grade) => grade.id === commandRank)
      ? commandRank
      : gradeForLevel(level).id;
    this.commandStats = Hero.#createCommandStats(commandStats);
    this.moraleHistory = Hero.#createMoraleHistory(moraleHistory);
    this.experience = Hero.#requireNonNegativeNumber(
      experience,
      "L'expérience",
    );
    ...
    this.specialPowerIds = Hero.#createUniqueIds(
      specialPowerIds,
      "Les pouvoirs du héros",
    );
    this.maxCommandPoints = Hero.#requirePositiveInteger(
      maxCommandPoints,
      "Les points de commandement maximum",
    );
    this.commandPoints = Hero.#requireCommandPoints(
      commandPoints,
      this.maxCommandPoints,
    );
    ...
    this.pendingLevelUps = structuredClone(
      Hero.#requireList(pendingLevelUps, "Les level-ups en attente"),
    );
    this.progressionHistory = structuredClone(
      Hero.#requireList(progressionHistory, "L'historique de progression"),
    );
    this.xpHistory = structuredClone(
      Hero.#requireList(xpHistory, "L'historique d'expérience"),
    );
    this.travelProgress = Hero.#createTravelProgress(travelProgress);
    this.appearanceId =
      appearanceId === null
        ? null
        : Hero.#requireText(appearanceId, "L'apparence du héros");
    if (
      classFeatureState === null ||
      Array.isArray(classFeatureState) ||
      typeof classFeatureState !== "object"
    )
      throw new TypeError("L'état des avantages de classe doit être un objet.");
  ...
  get maxUnitStacks() {
    return (
      HERO_GRADES.find((grade) => grade.id === this.commandRank)?.capacity ??
      HERO_GRADES[0].capacity
    );
  }
  get baseBagSlotCount() {
    const index = Math.max(
      0,
      HERO_GRADES.findIndex((grade) => grade.id === this.commandRank),
    );
    return [8, 10, 12, 14, 16][index] ?? 8;
  }
  get wagonSlotCount() {
    return this.wagons.reduce((sum, wagon) => sum + wagon.slotBonus, 0);
  }
  get bagSlotCount() {
    return this.baseBagSlotCount + this.wagonSlotCount;
  }
  get finalStats() {
    return Object.fromEntries(
      ["attack", "defense", "morale", "mobility", "command", "health"].map(
        (stat) => [
          stat,
          this.baseStats[stat] +
            this.statGrowth[stat] +
            this.equipmentModifiers[stat] +
            this.temporaryModifiers[stat],
        ],
      ),
    );
  }
  ...
  addExperience(amount, source = "legacy") {
    this.experience = Math.min(
      totalXpForLevel(HERO_MAX_LEVEL),
      this.experience +
        Hero.#requirePositiveNumber(amount, "Le gain d'expérience"),
    );
    this.xpHistory.push({
      type: "xp_gain",
      amount,
    ...
    return levelForExperience(this.experience) > this.level;
  }

  getResourceAmount(resourceName) {
    return (
      this.resources[
        Hero.#requireText(resourceName, "Le nom de la ressource")
      ] ?? 0
    );
  }
  addResource(resourceName, amount) {
    const name = Hero.#requireText(resourceName, "Le nom de la ressource");
    this.resources[name] =
      this.getResourceAmount(name) +
      Hero.#requirePositiveNumber(amount, "Le montant");
  }
  spendResource(resourceName, amount) {
    const name = Hero.#requireText(resourceName, "Le nom de la ressource");
    const value = Hero.#requirePositiveNumber(amount, "Le montant");
    if (this.getResourceAmount(name) < value) return false;
    this.resources[name] -= value;
    return true;
  }

  refreshCommandRank() {
    this.commandRank = gradeForLevel(this.level).id;
  ...
  setLevel(level) {
    const value = Hero.#requirePositiveInteger(level, "Le niveau");
    if (value > HERO_MAX_LEVEL)
      throw new RangeError("Le niveau maximal du héros est 20.");
    this.level = value;
  ...
  equip(slot, itemId) {
    ...
    this.equipment[validSlot] = Hero.#requireText(
      itemId,
  ...
  addCarriedLoot(entries) {
    this.carriedLoot.push(...Hero.#createCarriedLoot(entries));
  }
  ...
  addSkill(skillId) {
    return Hero.#addUniqueId(
      this.skillIds,
      skillId,
      "L'identifiant de la compétence",
    );
  }
  addSpecialPower(powerId) {
    return Hero.#addUniqueId(
      this.specialPowerIds,
      powerId,
      "L'identifiant du pouvoir",
    );
  }
  spendCommandPoints(cost) {
    const value = Hero.#requirePositiveInteger(cost, "Le coût de commandement");
    if (this.commandPoints < value) return false;
    this.commandPoints -= value;
    return true;
  }
  restoreCommandPoints(amount = this.maxCommandPoints) {
    const value = Hero.#requirePositiveInteger(
      amount,
      "La récupération de commandement",
    );
    this.commandPoints = Math.min(
      this.maxCommandPoints,
      this.commandPoints + value,
    );
  ...
  static #createPosition(position, allowNull = true) {
    ...
    if (
      ...
      throw new TypeError(
        "La position doit contenir une latitude et une longitude.",
      );
    }
    ...
    if (accuracy !== undefined) {
      normalizedPosition.accuracy = Hero.#requireNonNegativeNumber(
        accuracy,
        "La précision GPS",
      );
    }
    ...
    if (updatedAt !== undefined) {
      normalizedPosition.updatedAt = Hero.#requireText(
        updatedAt,
  ...
  static #createTravelProgress(progress) {
    if (
      ...
      throw new TypeError("La progression de déplacement doit être un objet.");
    return {
      totalDistanceMeters: Hero.#requireNonNegativeNumber(
        progress.totalDistanceMeters ?? 0,
        "La distance totale parcourue",
      ),
      remainderMeters: Hero.#requireNonNegativeNumber(
        progress.remainderMeters ?? 0,
        "La distance restante",
      ),
      lastPosition:
        progress.lastPosition == null
          ? null
          : Hero.#createPosition(progress.lastPosition, false),
    };
  }
  ...
  static #createResources(resources) {
    if (
      ...
      throw new TypeError("Les ressources du hÃ©ros doivent Ãªtre un objet.");
    if (Object.hasOwn(resources, "population"))
      throw new RangeError("La population n'est pas une ressource de hÃ©ros.");
    return Object.fromEntries(
      Object.entries(resources).map(([name, amount]) => [
        Hero.#requireText(name, "Le nom de la ressource"),
        Hero.#requireNonNegativeNumber(amount, "Une ressource initiale"),
      ]),
    );
  }
  ...
  static #createCommandStats(stats) {
    if (stats === null || Array.isArray(stats) || typeof stats !== "object")
      throw new TypeError("Les bonus de commandement doivent être un objet.");
    return Object.fromEntries(
      ["attackBonus", "defenseBonus", "moraleBonus"].map((name) => {
        const value = stats[name] ?? 0;
        if (!Number.isFinite(value))
          throw new RangeError(`Le bonus ${name} doit être un nombre.`);
        return [name, value];
      }),
    );
  }
  ...
  static #createStats(stats, label, defaultZero = false) {
    ...
    return Object.fromEntries(
      ["attack", "defense", "morale", "mobility", "command", "health"].map(
        (name) => {
          const value = stats[name] ?? (defaultZero ? 0 : undefined);
          if (!Number.isFinite(value))
            throw new RangeError(`${label} : ${name} doit être un nombre.`);
          return [name, value];
        },
      ),
    );
  ...
  static #createAptitudeRanks(ranks) {
    if (ranks === null || Array.isArray(ranks) || typeof ranks !== "object")
      throw new TypeError("Les rangs d'aptitude doivent être un objet.");
    return Object.fromEntries(
      Object.entries(ranks).map(([id, rank]) => [
        Hero.#requireText(id, "Une aptitude"),
        Hero.#requireText(rank, "Un rang"),
      ]),
    );
  }
  static #createTextList(values, label) {
    if (!Array.isArray(values))
      throw new TypeError(`${label} doit être une liste.`);
    return values.map((value) => Hero.#requireText(value, "Une valeur"));
  }
  static #requireList(values, label) {
    if (!Array.isArray(values))
      throw new TypeError(`${label} doit être une liste.`);
    return values;
  }
  ...
  static #createMoraleHistory(entries) {
    if (!Array.isArray(entries))
      throw new TypeError("L'historique de moral doit être une liste.");
    return entries.map((entry) => {
      const value = entry.value;
      if (!Number.isFinite(value))
        throw new RangeError("La variation de moral doit être un nombre.");
      return {
        source: Hero.#requireText(entry.source, "La source de moral"),
        value,
        reason:
          entry.reason === null || entry.reason === undefined
            ? null
            : Hero.#requireText(entry.reason, "La raison du moral"),
      };
    });
  }
  ...
  static #createCarriedLoot(entries) {
    if (!Array.isArray(entries))
      throw new TypeError("Le butin transporté doit être une liste.");
    return entries.map((entry, index) => ({
      id:
        entry.id === undefined
          ? `carried-${Hero.#requireText(entry.itemId, "L'objet transporté")}-${index}`
          : Hero.#requireText(entry.id, "L'identifiant du paquet transporté"),
      itemId: Hero.#requireText(entry.itemId, "L'objet transporté"),
      quantity: Hero.#requirePositiveInteger(
        entry.quantity,
        "La quantité transportée",
      ),
      valuePerUnit: Hero.#requirePositiveNumber(
        entry.valuePerUnit ?? 1,
  ...
  static #createWagons(wagons) {
    if (!Array.isArray(wagons))
      throw new TypeError("Les chariots doivent être une liste.");
    return wagons.map((wagon) => ({
      id: Hero.#requireText(wagon.id, "L'identifiant du chariot"),
      name: Hero.#requireText(wagon.name ?? "Chariot", "Le nom du chariot"),
      slotBonus: Hero.#requirePositiveInteger(
        wagon.slotBonus,
        "Les slots du chariot",
      ),
    }));
  }
  ...
  static #createUniqueIds(ids, label) {
    ...
    return [
      ...new Set(ids.map((id) => Hero.#requireText(id, "Un identifiant"))),
    ];
  }
  ...
  static #requireHealth(value, maxHealth, allowZero = false) {
    if (
      ...
      throw new RangeError(
        "Les PV doivent être compris entre zéro et les PV maximum.",
      );
    }
    return value;
  }
  static #requireCommandPoints(value, maximum) {
    if (!Number.isInteger(value) || value < 0 || value > maximum)
      throw new RangeError(
        "Les points de commandement doivent être compris entre zéro et leur maximum.",
      );
    return value;
  }
}
