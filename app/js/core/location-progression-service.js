...
export class LocationProgressionService {
  constructor({ mode = "casual" } = {}) {
    if (!["casual", "expert"].includes(mode)) throw new RangeError("Le mode des lieux doit être casual ou expert.");
    this.mode = mode;
  }
  ...
  initialize(location) {
    ...
    location.durability.health = Math.min(maxHealth, location.durability.health);
  ...
  getMaxHealth(location) {
    ...
    const improvements = Object.values(location.infrastructure ?? {}).reduce((sum, level) => sum + level, 0);
    return definition.baseHealth + definition.healthPerLevel * location.level + improvements * 25;
  }

  getExperienceRequired(location) {
    const definition = LOCATION_EVOLUTION[location.type];
    return definition ? Math.round(definition.xpBase * location.level ** 1.35) : null;
  }

  awardExperience(location, amount, source = "activity") {
    if (!Number.isFinite(amount) || amount <= 0) throw new RangeError("L'expérience gagnée doit être positive.");
    location.progression.experience += amount;
    location.statistics[`xp_${source}`] = (location.statistics[`xp_${source}`] ?? 0) + amount;
    return { experience: location.progression.experience, required: this.getExperienceRequired(location) };
  }
  ...
  applyAttack(location, attackPoints) {
    if (!Number.isFinite(attackPoints) || attackPoints <= 0) throw new RangeError("Les dégâts d'attaque doivent être positifs.");
    const durability = this.initialize(location);
    if (!durability) return { applied: false, reason: "indestructible_in_this_mode" };
    let remainingDamage = attackPoints;
    const lostLevels = [];
    while (remainingDamage >= durability.health && location.state !== "destroyed") {
    ...
    return { applied: true, damage: attackPoints, lostLevels, destroyed: location.state === "destroyed", health: durability.health, maxHealth: durability.maxHealth };
  }

  reconstruct(location, hours = 1) {
    if (!Number.isFinite(hours) || hours <= 0) throw new RangeError("La durée de reconstruction doit être positive.");
    const durability = this.initialize(location);
    if (!durability || location.state === "destroyed" || durability.health >= durability.maxHealth) return { restored: 0, spent: {} };
    ...
    if (!materialId || (stock.gold ?? 0) <= 0) return { restored: 0, spent: {} };
    const affordable = Math.floor(Math.min((stock[materialId] ?? 0) * 5, stock.gold * 20));
    const restored = Math.min(potential, affordable, durability.maxHealth - durability.health);
    if (restored <= 0) return { restored: 0, spent: {} };
    const materials = restored / 5; const gold = restored / 20;
    stock[materialId] -= materials; stock.gold -= gold; durability.health += restored;
