...
export class SetupPlacementService {
  ...
  generate({ playArea, count, occupied = [], minimumDistance = 0 }) {
    if (!Number.isInteger(count) || count < 0)
      throw new RangeError(
        "Le nombre de placements doit être un entier positif ou nul.",
      );
    ...
    while (selected.length < count) {
      ...
      const ranked = candidates
        .filter(
          (candidate) =>
            !selected.some(
              (position) => this.distanceFn(position, candidate) === 0,
            ),
        )
        .map((candidate) => ({
          candidate,
          distance:
            references.length === 0
              ? Infinity
              : Math.min(
                  ...references.map((position) =>
                    this.distanceFn(position, candidate),
                  ),
                ),
        }))
        .sort((a, b) => b.distance - a.distance);
      const choice =
        ranked.find((entry) => entry.distance >= minimumDistance) ?? ranked[0];
      if (!choice)
        throw new Error(
          "La zone ne contient pas assez d’emplacements distincts.",
        );
  ...
  resolveInside({ playArea, preferred, origin = null, minimumDistance = 0 }) {
    ...
    const ranked = this.#candidates(playArea)
      .filter(
        (candidate) =>
          origin === null ||
          this.distanceFn(origin, candidate) >= minimumDistance,
      )
      .map((candidate) => ({
        candidate,
        distance: this.distanceFn(safePreferred, candidate),
      }))
      .sort((first, second) => first.distance - second.distance);
    if (ranked.length === 0)
      throw new RangeError(
        "La zone de jeu est trop petite pour placer cet objectif à la distance demandée.",
      );
  ...
  findPosition({
    ...
    return {
      ...candidates
        .map((candidate) => {
          const distance = this.distanceFn(origin, candidate);
          const bearing = bearingDegrees(origin, candidate);
          const distanceError = Math.abs(distance - preferredDistance);
          const directionError = angularDistance(direction, bearing);
          const score =
            distanceError +
            (directionError / 180) * Math.max(preferredDistance, 1);
          return { candidate, score, distanceError, directionError };
        })
        .sort(
          (first, second) =>
            first.score - second.score ||
            first.distanceError - second.distanceError ||
            first.directionError - second.directionError,
        )[0].candidate,
    };
  }

  #candidates(playArea) {
    const latitudes = playArea.polygon.map((point) => point.latitude);
    const longitudes = playArea.polygon.map((point) => point.longitude);
...
function clampGeographicPosition(position) {
  if (
    ...
    throw new TypeError(
      "La position préférée doit contenir une latitude et une longitude finies.",
    );
  }
  return {
...
export function bearingDegrees(origin, target) {
  const averageLatitude =
    (((origin.latitude + target.latitude) / 2) * Math.PI) / 180;
  const east =
    (target.longitude - origin.longitude) * Math.cos(averageLatitude);
