...
const setup = {
  ...
  rules: { maxUnitsPerHero: 3 },
};
const scenario = {
  id: "chaos",
  name: "Chaos",
  intro: "Fuyez.",
  initialPhaseId: "escape",
...
const locations = [
  {
    ...
    defenseSlots: 2,
    features: { recruitment: true, healing: true, garrison: true },
    recruitment: {
      availableUnitTypeIds: ["archer"],
      stock: { archer: 20 },
      production: { archer: 2 },
      capacity: 30,
      variance: 0.25,
...
test("le début attribue les ressources de classe et scénario, puis recrute au fort", () => {
  ...
  const hero = game.chooseHero("player-1", {
    name: "Aldric",
    classId: "warrior",
  });
  game.start();
  ...
  const secondResult = game.recruitUnit({
    playerId: player.id,
    heroId: hero.id,
    locationId: "fort-1",
    unitTypeId: "archer",
...
test("la disponibilité explique pourquoi un recrutement serait refusé", () => {
  const game = new Game({
    setup,
    scenario,
    heroClasses,
  ...
  assert.deepEqual(
    game.getRecruitmentAvailability({
      playerId: "player-1",
      heroId: hero.id,
      locationId: fort.id,
      unitTypeId: "archer",
  ...
  hero.spendResource("gold", hero.getResourceAmount("gold"));
  assert.deepEqual(
    game.getRecruitmentAvailability({
      playerId: "player-1",
      heroId: hero.id,
  ...
  fort.recruitment.stock.archer = 0;
  assert.equal(
    game.getRecruitmentAvailability({
      playerId: "player-1",
      heroId: hero.id,
...
test("un héros peut dissoudre définitivement une unité hors combat", () => {
  ...
  assert.ok(
    game.eventLog.some(
      (event) => event.type === "unit_disbanded" && event.unitId === unit.id,
    ),
  );
  assert.deepEqual(
    game.disbandUnit({
    ...
    { success: false, reason: "invalid_unit" },
  );
});
...
test("un lieu complète les unités avec son stock de recrues compatibles", () => {
  ...
  unit.lose(2);
  const stockBefore = fort.recruitment.stock.archer;
  ...
  assert.equal(result.success, true);
  assert.equal(unit.quantity, unit.maxQuantity);
  assert.equal(fort.recruitment.stock.archer, stockBefore - 2);
  assert.deepEqual(result.costs, { gold: 10, wood: 4, iron: 2 });
  assert.equal(hero.getResourceAmount("gold"), 20);
});
...
test("chaque soldat survivant récupère un PV par unité de temps dans un lieu de soin", () => {
  ...
  assert.equal(result.restoredHealth, unit.quantity);
  assert.deepEqual(result.costs, { gold: unit.quantity });
  assert.equal(hero.getResourceAmount("gold"), 30 - unit.quantity);
  assert.ok(
    unit.soldierHealth.every((health) => health === unit.healthPerSoldier - 1),
  );
});

test("renforts et soins ne modifient rien quand le héros ne peut pas payer", () => {
  const game = new Game({ setup, scenario, heroClasses, unitDefinitions, locations });
  ...
  unit.lose(2);
  unit.soldierHealth = unit.soldierHealth.map((health) => health - 1);
  hero.spendResource("gold", hero.getResourceAmount("gold"));
  const stockBefore = fort.recruitment.stock.archer;
  const healthBefore = [...unit.soldierHealth];

  assert.equal(
    game.completeHeroUnits({
      playerId: "player-1",
      heroId: hero.id,
      locationId: fort.id,
    }).reason,
  ...
  assert.equal(unit.quantity, 4);
  assert.equal(fort.recruitment.stock.archer, stockBefore);
  assert.equal(
    game.healHeroUnits({
      playerId: "player-1",
      heroId: hero.id,
  ...
  assert.deepEqual(unit.soldierHealth, healthBefore);
});
...
test("les unités récupèrent passivement à chaque cycle hors des localisations", () => {
  ...
  fort.removeHero(hero.id);
  hero.updatePosition({ latitude: 5, longitude: 5 });
  unit.soldierHealth = unit.soldierHealth.map((health) => health - 2);
